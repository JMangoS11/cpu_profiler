...
u64 timespec_diff_to_ns(struct timespec *start, struct timespec *end) {
    u64 start_ns = static_cast<u64>(start->tv_sec) * 1000000000LL + start->tv_nsec;
    u64 end_ns = static_cast<u64>(end->tv_sec) * 1000000000LL + end->tv_nsec;
...
void getFinalizedData(int numthreads,double profile_time,std::vector<raw_data>& data_begin,std::vector<raw_data>& data_end,std::vector<profiled_data>& result_arr,std::vector<thread_args*> thread_arg){
  for (int i = 0; i < numthreads; i++) {
	...
	//TODO-change time frequency
        result_arr[i].capacity_adj = ((profile_time*1000000)/perf_use) * data_end[i].raw_compute;
        std::cout<<"use factor"<<perf_use<<std::endl;
      }
...
void* run_computation(void * arg)
{
    ...
    while(true) {
      ...
      struct timespec actualstart,actualend;	
      ...
      if (profiler_iter % heavy_profile_interval == 0){
        clock_gettime(CLOCK_THREAD_CPUTIME_ID, &starts);
        clock_gettime(CLOCK_MONOTONIC,&actualstart);
        heavy_interval = true;
      }
      ...
      *args->addition_calc = addition_calculator;
      if(heavy_interval){
        clock_gettime(CLOCK_THREAD_CPUTIME_ID, &ends);
	clock_gettime(CLOCK_MONOTONIC,&actualend);
	std::cout<<"BAAA"<< timespec_diff_to_ns(&starts,&ends) <<" GA"<<timespec_diff_to_ns(&actualstart,&actualend)<<"A ";
        args->user_time =static_cast<double>(timespec_diff_to_ns(&starts,&ends))/static_cast<double>(timespec_diff_to_ns(&actualstart,&actualend));
	*args->addition_calc = timespec_diff_to_ns(&actualstart,&actualend);
     	}
	 initialized = 0;
      }
      return NULL;
