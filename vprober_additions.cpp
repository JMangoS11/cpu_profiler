...
void do_profile(std::vector<raw_data>& data_end,std::vector<thread_args*> thread_arg){
    ...
    while(true){
      ...
      //If the last interval was heavy, move the threads to low priority. If the interval is one, we're looking at a workload event so just ignore.
      if ((heavy_profile_interval>1) && ((profiler_iter-1) % heavy_profile_interval == 0)){
