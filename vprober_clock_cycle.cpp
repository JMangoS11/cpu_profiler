...
void moveThreadtoLowPrio(pid_t tid) { //commenting all uses -testing-
...
void banVcpus(std::vector<profiled_data>& data_arr){ //commented all refrences
...
void updateVectorFromBanlist(std::string fileLocation) { //commented all refrences
    std::ifstream file(fileLocation);

    if (!file) {
        std::cerr << "Failed to access file " <<fileLocation << std::endl;
        return;
    }
...
void do_profile(std::vector<raw_data>& data_end,std::vector<thread_args*> thread_arg){
	...
	//profiler_iter++; //Testing
      }
      //updateVectorFromBanlist("/home/jmangos/banlist/vtop.txt");
      //sleep during sleep
